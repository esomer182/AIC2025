...
public class Shoveler extends Unit{
    Location myLoc;
    Craftable weapon = Craftable.PICKAXE;
    Location obj = null;
    int randomObj = 1;
    UnitInfo me;
    Material[] wantedMaterials = {Material.GRASS, Material.POTATO};
    int round;
    public Shoveler (UnitController uc) {
        init(uc);
        getRandomLoc();
        me = uc.getUnitInfo();
    }
    ...
    public void play() {
        ...
        readBuffer();
        updBuffer();
        if (uc.senseStructures(GameConstants.UNIT_VISION_RANGE, uc.getOpponent(), StructureType.BED).length>0) changeType ("AT", "S");
        else if (uc.senseUnits(2, uc.getOpponent()).length>0) changeType ("AT", "S");
        else{
