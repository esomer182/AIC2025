...
public class Pickaxer extends Unit{
    Location myLoc;
    Craftable weapon = Craftable.PICKAXE;
    Location obj = null;
    int randomObj = 1;
    UnitInfo me;
    Material[] wantedMaterials = {Material.STONE, Material.COPPER, Material.IRON, Material.GOLD, Material.DIAMOND};
    int round;
    public Pickaxer (UnitController uc) {
        init(uc);
        getRandomLoc();
        me = uc.getUnitInfo();
    }
    ...
    public void play() {
        ...
        readBuffer();
        updBuffer();
        if (uc.senseStructures(GameConstants.UNIT_VISION_RANGE, uc.getOpponent(), StructureType.BED).length>0) changeType ("AT", "P");
        else if (uc.senseUnits(2, uc.getOpponent()).length>0) changeType ("AT", "P");
        else{
