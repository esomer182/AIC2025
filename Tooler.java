...
public class Tooler extends Unit{
    ...
    public void play() {
        ...
        searchLocation();
        if (randomObj != 1 && uc.canSenseLocation(obj) && uc.senseMaterialAtLocation(obj).gatheringTool() != null
                && uc.hasCraftable(uc.senseMaterialAtLocation(obj).gatheringTool())
                && uc.canUseCraftable(uc.senseMaterialAtLocation(obj).gatheringTool(), obj)) {
        ...
        } else if (randomObj == 1 && uc.canGather()) {
            uc.gather();
        } else if (myLoc.isEqual(obj) && uc.canGather()) {
            uc.gather();
        }
    ...
    public void tryCraftTools() {
        if (uc.getRound() >= 250 && uc.canCraft(Craftable.SHOVEL) && !uc.hasCraftable(Craftable.SHOVEL)) {
            uc.craft(Craftable.SHOVEL);
        }
        if (uc.getRound() >= 500 && uc.canCraft(Craftable.PICKAXE) && !uc.hasCraftable(Craftable.PICKAXE)) {
